...
O código correto é **CODIGO_IBGE_MUNICIPIO** (Ipojuca/PE), que já foi corrigido no código.
...
1. Verifique se o CNPJ `SEU_CNPJ` tem a inscrição municipal `SUA_IM` cadastrada
2. Verifique se está vinculada ao município `CODIGO_IBGE_MUNICIPIO` (Ipojuca/PE)
...
```json
{
  "prestador": {
    ...
    "inscricao_municipal": "SUA_IM"  // Adicionar mesmo sendo opcional
  }
}
```
...
- Município: Ipojuca/PE (Código IBGE: CODIGO_IBGE_MUNICIPIO)
