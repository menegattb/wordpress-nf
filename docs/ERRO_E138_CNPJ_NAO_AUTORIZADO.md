...
O CNPJ `51581345000117` precisa estar cadastrado e autorizado na sua conta da Focus NFe especificamente para emitir NFSe.

**Solução:**
1. Acesse o painel da Focus NFe: https://app.focusnfe.com.br
2. Vá em **Configurações** → **CNPJs**
3. Verifique se o CNPJ `51581345000117` está cadastrado
...
O código do município pode estar incorreto. O código correto é `2607208` (Ipojuca/PE).

**Solução:**
1. Verifique se a variável `PRESTADOR_MUNICIPIO=2607208` está configurada no `.env`
2. O código foi corrigido no `config.js` para usar `2607208` como padrão
...
# Código do município (deve ser 2607208)
...
2. Vá em **Configurações** → **CNPJs**
3. Verifique:
   - ✅ CNPJ `51581345000117` está cadastrado
...
1. No painel da Focus NFe, adicione o CNPJ `51581345000117`
...
O código do município foi corrigido de `2607200` para `2607208` no arquivo `config.js`.
