...
async function atualizarStatus(req, res) {
  try {
    ...
    const pedido = await buscarPedidoPorPedidoId(id, req.tenant_id);
...
/**
 * Atualiza endereço de um pedido
 */
async function atualizarEndereco(req, res) {
  try {
    const { id } = req.params; // Este pode ser pedido_id (WooCommerce) ou id (Excel)
    ...
    // Primeiro tentar buscar como pedido do WooCommerce (pedido_id)
    let pedido = await buscarPedidoPorPedidoId(id, req.tenant_id);
    
    if (!pedido) {
      // Se não encontrou, pode ser um pedido do Excel que não está no banco
      // Retornar erro informando que deve usar a rota do Excel
      return res.status(404).json({
        erro: 'Pedido não encontrado. Para pedidos do Excel, use /api/excel/pedidos/:pedido_id/endereco'
      });
    }
    
    // Verificar se o pedido é do WooCommerce
    const isWooCommerce = pedido.origem === 'woocommerce';
    
    // Se for do WooCommerce, atualizar também no WooCommerce
    if (isWooCommerce) {
      try {
        ...
        logger.info('Atualizando endereço no WooCommerce', {
          pedido_id: id,
          origem: 'woocommerce'
        });
        
        const resultadoWC = await woocommerce.atualizarEnderecoPedido(
          id,
          endereco,
          cpf_cnpj,
          credentials
        ...
        if (!resultadoWC.sucesso) {
          logger.warn('Falha ao atualizar endereço no WooCommerce, continuando com atualização local', {
            pedido_id: id,
            erro: resultadoWC.erro
          });
          // Continuar mesmo se falhar no WooCommerce, para atualizar no banco local
        } else {
          logger.info('Endereço atualizado com sucesso no WooCommerce', {
            pedido_id: id
          });
        }
      } catch (errorWC) {
        logger.error('Erro ao atualizar endereço no WooCommerce', {
          pedido_id: id,
          error: errorWC.message
        });
        // Continuar mesmo se falhar no WooCommerce, para atualizar no banco local
      }
    }
    
    // Preparar dados para atualização no banco local
    const dadosPedido = pedido.dados_pedido || {};
    const dadosPedidoAtualizado = {
      ...dadosPedido,
      endereco: endereco,
      ...(nif !== undefined && { nif }),
      ...(cpf_cnpj !== undefined && { cpf_cnpj })
    };
    
    const atualizacoes = {
      dados_pedido: dadosPedidoAtualizado
    };
    
    logger.info('Atualizando pedido no banco local', {
      pedido_id: id,
      id_interno: pedido.id,
      origem: pedido.origem,
      endereco_antigo: dadosPedido.endereco?.cidade || dadosPedido.endereco?.nome_cidade_ext,
      endereco_novo: endereco.cidade || endereco.nome_cidade_ext
    });
    
    // Atualizar usando o ID interno
    const pedidoAtualizado = await atualizarPedido(pedido.id, atualizacoes);
    
    if (!pedidoAtualizado) {
      logger.error('Falha ao atualizar pedido no banco', {
        pedido_id: id,
        id_interno: pedido.id
      });
      return res.status(500).json({
        erro: 'Falha ao atualizar pedido no banco de dados'
      });
    }
    
    logger.info('Pedido atualizado com sucesso no banco local', {
      pedido_id: id,
      id_interno: pedido.id
    });
    ...
    res.json({
      sucesso: true,
      pedido: pedidoAtualizado,
      atualizado_no_woocommerce: isWooCommerce
    });
    
  } catch (error) {
    logger.error('Erro ao atualizar endereço do pedido', {
      error: error.message
    });
  ...
  try {
    ...
    for (const pedido of pedidos) {
      try {
        ...
        const existente = await buscarPedidoPorPedidoId(pedidoId, tenantId);
...
async function listarPedidosBanco(req, res) {
  try {
    ...
    // Extrair dados_pedido para formato mais útil (listarPedidos retorna array)
    const pedidos = (Array.isArray(resultado) ? resultado : (resultado.dados || [])).map(p => {
