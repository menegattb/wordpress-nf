...
function createApiClient(configOverride = null) {
  ...
  axiosRetry(client, {
    ...
    onRetry: (retryCount, error, config) => {
      logger.warn('Retry Focus NFe (NFe)', {
        service: 'focusNFe',
        action: 'retry',
        retryCount,
        url: config?.url,
 ...
 */
async function emitirNFe(dadosPedido, configEmitente, configFiscal = null, configFocus = null) {
  let apiConfig = null;
  try {
  ...
  try {
    ...
    if (isAlreadyProcessed) {
      ...
      try {
        const notaExistente = await consultarNFe(referencia, configFocus);
 ...
 * @param {Object} configFocus - Opcional: { token, ambiente } para multi-tenant
 */
async function consultarNFe(referencia, configFocus = null) {
 ...
 * @param {Object} configFocus - Opcional: { token, ambiente } para multi-tenant
 */
async function cancelarNFe(referencia, justificativa, configFocus = null) {
 ...
 */
async function listarTodasNFe(filtros = {}, configFocus = null) {
