...
```bash
./cancelar_nota_por_chave.sh CHAVE_NFE_EXEMPLO "Nota emitida por engano, necessário cancelamento" producao
```

## Método 2: Usando cURL Diretamente

```bash
curl -X DELETE "http://localhost:3000/api/nfse/cancelar-por-chave/CHAVE_NFE_EXEMPLO" \
...
2. Busque pela chave da nota: `CHAVE_NFE_EXEMPLO`
...
{
  "sucesso": false,
  "erro": "Nota com chave CHAVE_NFE_EXEMPLO não encontrada no banco local nem na Focus NFe"
}
```
